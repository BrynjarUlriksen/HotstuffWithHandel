...
type Hotstuff interface {
	...
	RequestHandelCertificate(ctx gorums.ServerCtx, request *HandelSignature)
...
func RegisterHotstuffServer(srv *gorums.Server, impl Hotstuff) {
	...
	srv.RegisterHandler("hotstuffpb.Hotstuff.RequestHandelCertificate", func(ctx gorums.ServerCtx, in *gorums.Message, _ chan<- *gorums.Message) {
		req := in.Message.(*HandelSignature)
		defer ctx.Release()
		impl.RequestHandelCertificate(ctx, req)
	})
...
// RequestHandelCertificate is a quorum call invoked on all nodes in configuration c,
// with the same argument in, and returns a combined result.
func (n *Node) RequestHandelCertificate(ctx context.Context, in *HandelSignature, opts ...gorums.CallOption) {
	cd := gorums.CallData{
		Message: in,
		Method:  "hotstuffpb.Hotstuff.RequestHandelCertificate",
	}
